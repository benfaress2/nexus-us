...
global.db = new Low(/https?:\/\//.test(opts['db'] || '') ? new cloudDBAdapter(opts['db']) : new JSONFile(`${opts._[0] ? opts._[0] + '_' : ''}nexus/database.json`))
...
global.authFolder = storeSys.fixFileName(`${opts._[0] || ''}nexus/sessions`)
let {
    state,
    saveCreds
} = await useMultiFileAuthState(path.resolve('./nexus/sessions'))
...
const directory = './nexus/sessions';
...
async function connectionUpdate(update) {
    ...
    if (connection === "open") {
    console.log(chalk.bgGreen(chalk.white('بوت اشتغل بناج ✨️')));
    }
    if (connection == 'close') {
